  ...
  const getUserDetails = async () => {
    try {
      ...
      if (response.ok) {
        ...
        let current_pfp = data.pfp.split("/")[3].split(".")[0];
        setSelectedPfp(current_pfp);
      } else {
  ...
  const updatePfp = async () => {
    ...
    try {
      const response = await fetch("/user/updatePfp", {
        ...
        body: JSON.stringify({ pfp: `/images/icons/${selectedPfp}.png` }),
      });
      ...
      <div id="account-settings">
        ...
        {userSettings !== null ? (
          <>
            ...
            <h3>Monthly Email Newsletter</h3>
            <div id="email-prefs">
              <label class="switch">
                <input
                  type="checkbox"
            ...
            <div id="pfp-pref">
              ...
              <div id="settings-pfps">
                <button
                  id="pfp"
                  className={selectedPfp === "racoon" ? "selected" : ""}
                  onClick={() => setSelectedPfp("racoon")}
                >
                  <img src="/images/icons/racoon.png" />
                </button>
                <button
                  id="pfp"
                  className={selectedPfp === "lion" ? "selected" : ""}
                  onClick={() => setSelectedPfp("lion")}
                >
                  <img src="/images/icons/lion.png" />
                </button>
                <button
                  id="pfp"
                  className={selectedPfp === "platypus" ? "selected" : ""}
                  onClick={() => setSelectedPfp("platypus")}
                >
                  <img src="/images/icons/platypus.png" />
                </button>

                <button
                  id="pfp"
                  className={selectedPfp === "bear" ? "selected" : ""}
                  onClick={() => setSelectedPfp("bear")}
                >
                  <img src="/images/icons/bear.png" />
                </button>
              </div>
