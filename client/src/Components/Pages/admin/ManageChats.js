...
const ManageChats = () => {
  ...
  const getCurrentPage = () => {
    let pages = {"flagged-chats" : <FlaggedChats/>, "chatView":<AdminChatView user_id={userView}/>}
    return pages[page];
  }

  const setViewing = async(user_id) => {
    setUserView(user_id);
    setPage("chatView")
  }
  ...
  return (
   <div className="manage-chats">
     {page == "home" ? <>
     <button id="flagged-chats" onClick={()=>setPage('flagged-chats')}>See Flagged</button>
     <h1>View Activity By User</h1>
     Username: <input type="text" onChange={(e)=>setSearch(e.target.value)}></input>
     <ul>
        {filtered_usernames ? <> {filtered_usernames.map((user, index) => (
            <li key={index}>
            <p>{user.username}</p>
            <button onClick={() => setViewing(user.id)}>View Activity</button>
            <button onClick={() => disableUser(user.id)}>Disable Sender Priviliges</button>
        </li>
            ))}</> : <></>}
     </ul>
