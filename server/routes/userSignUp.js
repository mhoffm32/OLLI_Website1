...
router.route("/signup").post(checkForBadWords, async (req, res) => {
  console.log("Signing Up");
  const { email, username, password } = req.body;
  const usernameLower = username.toLowerCase().trim();
  const emailLower = email.toLowerCase().trim();
  if (
    (await InputChecker.validateEmail(emailLower)) &&
    InputChecker.sanitizeInput(username) &&
    InputChecker.sanitizeInput(password)
  ) {
    if (
      !(await UserInterface.getUserByEmail(emailLower)) &&
      !(await UserInterface.getUserByUsername(usernameLower))
    ) {
...
router.route("/user/resendVerification").post(async (req, res) => {
...
async function createUser(username, password, email, res) {
  ...
  if (result) {
    ...
    if (!result.verified) sendVerificationEmail(newUser, res);
  }
